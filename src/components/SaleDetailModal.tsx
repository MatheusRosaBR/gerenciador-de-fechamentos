...
interface SaleDetailModalProps {
  sale: SaleContract;
  onClose: () => void;
  onUpdateStatus: (saleId: string, newStatus: ReceiptStatus) => void;
  onToggleReminder: (saleId: string) => void;
}
  ...
  <div className="flex items-start gap-3">
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="flex items-center justify-between gap-2">
        <p className="font-semibold text-[var(--color-text-primary)] break-words overflow-wrap-anywhere max-w-full">{value}</p>
        {children}
      </div>
    </div>
  </div>
...
const SaleDetailModal: React.FC<SaleDetailModalProps> = ({ sale, onClose, onUpdateStatus, onToggleReminder }) => {
  const handleStatusChange = (newStatus: ReceiptStatus) => {
    onUpdateStatus(sale.id, newStatus);
  };
    ...
    >
      ...
      >
        <div className="p-6 border-b border-[var(--color-border)] flex justify-between items-center">
          <h2 className="text-xl font-bold text-[var(--color-text-primary)]">Detalhes da Venda</h2>
        ...
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
          <DetailItem icon={<UserIcon className="w-5 h-5" />} label="Cliente" value={sale.cliente} />
          <DetailItem icon={<HomeIcon className="w-5 h-5" />} label="Imóvel" value={sale.imovel} />
          <DetailItem icon={<PhoneIcon className="w-5 h-5" />} label="Telefone" value={sale.telefone || 'Não informado'} />
          <DetailItem icon={<MailIcon className="w-5 h-5" />} label="Email" value={sale.email || 'Não informado'} />
          <DetailItem icon={<CurrencyDollarIcon className="w-5 h-5" />} label="Valor da Venda" value={formatCurrencyBRL(sale.valorVenda)} />
        ...
        <div className="p-6 bg-[var(--color-bg-muted)]/50 rounded-b-xl flex flex-col gap-4">
          <div>
            <h3 className="text-sm font-medium text-[var(--color-text-secondary)] mb-3">Status de Pagamento da Comissão</h3>
          ...
          {sale.statusRecebimento === ReceiptStatus.Nao && (
            <div>
              ...
              >
                ...
                {sale.lembreteAtivo ? 'Lembrete Ativado' : 'Ativar Lembrete'}
              </button>
            </div>
          )}
        </div>
