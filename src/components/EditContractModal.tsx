...
const EditContractModal: React.FC<EditContractModalProps> = ({ contract, onClose, onUpdateContract, onDeleteContract }) => {
  useLockBodyScroll();
  const [formData, setFormData] = useState({
    ...contract,
    valorLocacao: maskCurrency(contract.valorLocacao.toFixed(2)),
    comissao: maskCurrency(contract.comissao.toFixed(2)),
    telefone: maskPhone(contract.telefone || ''),
  });
  ...
  const validate = () => {
    ...
    const valorLocacaoNum = parseCurrency(formData.valorLocacao as string);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      const valorLocacao = parseCurrency(formData.valorLocacao as string);
      ...
      onUpdateContract({
        ...
        percentualComissao: valorLocacao > 0 ? comissao / valorLocacao : 0,
      });
    }
  };
    ...
    >
      ...
      >
        <div className="p-6 border-b border-[var(--color-border)] flex justify-between items-center">
          <h2 id="edit-contract-title" className="text-xl font-bold text-[var(--color-text-primary)]">Editar Contrato</h2>
